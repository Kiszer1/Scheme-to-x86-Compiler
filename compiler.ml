...
module Reader : READER = struct
  ...
  and nt_paired_comment str = 
    let nt1 = char '{' in
    let nt2 = char '}' in 
    let nt3 = unitify(disj nt_string nt_char) in
    let nt4 = unitify(diff nt_any (disj nt1 nt2)) in
    let nt3 = disj nt3 nt4 in
    let nt3 = star(disj nt3 nt_paired_comment) in
    let nt1 = unitify(caten nt1 (caten nt3 nt2)) in
    nt1 str
  ...
  and nt_hex_digit str = 
    ...
    let nt2 = pack nt2 (fun ch -> int_of_char ch - delta) in
    let nt1 = disj nt1 nt2 in
  ...
  and nt_float str = 
    ...
    let nt2 = caten nt_integer_part
                (caten (char '.')
                  (caten (make_maybe nt_mantissa 0.0)
                    (make_maybe nt_exponent 1.00))) in
    let nt2 = pack nt2 (fun (ip, (_ , ( mant, mult))) ->
                (ip +. mant) *.mult) in
    let nt3 = caten (char '.')
                (caten nt_mantissa 
    ...
    let nt1 = pack (caten nt1 nt2) (fun (is_positive, x) ->
                  if is_positive then x else (-. x)) in
    let nt1 = pack nt1 (fun real -> ScmReal real) in
    nt1 str
  and nt_number str =
    let nt1 = nt_float in
    let nt2 = nt_frac in
  ...
  and nt_char_named str =
    let nt1 =
      disj_list [pack (word_ci "space") (fun _ -> ' ');
  ...
  and nt_symbol str = 
    ...
    let nt1 = pack nt1 (fun symb -> ScmSymbol (string_of_list symb)) in
    nt1 str
  ...
  and nt_string_part_dynamic str = 
    let nt1 = word "~{" in
    let nt1 = unitify (caten nt1 nt_skip_star) in
    let nt2 = nt_sexpr in
    let nt1 = pack (caten nt1 (caten nt2 (unitify (char '}'))))
                   (fun (_, (expr, _)) -> 
                    Dynamic (ScmPair(ScmSymbol "format", ScmPair(ScmString "~a", ScmPair(expr, ScmNil))))) in
    nt1 str
  ...
  and nt_vector str = 
    let nt1 = word "#(" in
    let nt2 = pack (caten (nt_skip_star) (char ')')) (fun _ -> []) in
    let nt3 = plus nt_sexpr in
    let nt3 = pack (caten nt3 (char ')')) (fun (sexprs, _) -> List.fold_right (fun left right -> left::right) sexprs []) in
    let nt2 = disj nt3 nt2 in 
    let nt1 = pack (caten nt1 nt2) (fun (_,sexprs) -> sexprs) in
    let nt1 = pack nt1 (fun vector -> ScmVector vector) in
    nt1 str
  and nt_list str = 
    let nt1 = char '(' in
...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec macro_expand_and_clauses expr = function
    | [] -> expr
    | expr' :: exprs ->
    	let ands = macro_expand_and_clauses expr' exprs in
    	ScmPair
   			(ScmSymbol "if",
    			ScmPair
     				(expr,
      				ScmPair (ands, ScmPair (ScmBoolean false, ScmNil))));;
    ...
    | ScmPair (ScmSymbol "let", ScmPair (ribs, exprs)) ->
              (match (scheme_list_to_ocaml ribs) with
                ...
                | ribs, ScmNil ->
                     let (params, args) =
                        List.fold_right 
                           (fun rib (lastParam, lastArg) ->
                              match rib with
                                 | ScmPair(param, ScmPair(arg, ScmNil)) ->
                                    (ScmPair(param, lastParam), ScmPair(arg, lastArg))
                                 | _ -> raise (X_syntax "invalid let ribs"))
                           ribs
                           (ScmNil, ScmNil) in
                     tag_parse (ScmPair (ScmPair (ScmSymbol "lambda", ScmPair(params, exprs)),args))
               | _ -> raise (X_syntax "invalid let"))
    ...
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
         (match (scheme_list_to_ocaml ribs) with
            ...
            | ribs, ScmNil ->
               let (params, sets) =
                        List.fold_right 
                           (fun rib (lastParam, lastSet) ->
                              match rib with
                                 | ScmPair(param, arg) ->
                                    (ScmPair (ScmPair (param, ScmPair (ScmPair (ScmSymbol "quote", ScmPair (ScmSymbol  "whatever", ScmNil)), ScmNil)), lastParam), 
                                      ScmPair (ScmPair(ScmSymbol ("set!"), ScmPair(param, arg)), lastSet))
                                 | _ -> raise (X_syntax "invalid letrec ribs"))
                           ribs
                           (ScmNil, exprs) in
               tag_parse (ScmPair (ScmSymbol "let", ScmPair(params, sets)))
            | _ -> raise (X_syntax "invalid letrec")
         )
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let should_box_var name expr params = 
    let (as', bs') = find_reads_and_writes name expr params in
    let products = cross_product as' bs' in
    let prod = List.filter (fun ((v1, env1),(v2, env2)) ->
         match v1 with
            | Var' (_, Bound (major, _)) -> 
               (match v2 with
                  | Var' (_, Param _) -> true
  ...
  let rec auto_box expr =
    ...
    | ScmOr' exprs -> ScmOr'(List.map (fun expr -> auto_box expr) exprs)
    | ScmLambda' (params, Simple, expr') ->
       let box_these =
         List.filter
  ...
  | ScmApplic' (proc, args, app_kind) ->
     ...
     let args =
       Reader.scheme_sexpr_list_of_sexpr_list
         (List.map sexpr_of_expr' args) in
     ScmPair (proc, args)
  | _ -> raise (X_syntax "the expr' is not valid")
...
module Code_Generation : CODE_GENERATION= struct
  ...
  let remove_duplicates = 
    let rec run sexpr sexprs =
      [sexpr] @ runs (List.filter (fun sexpr' -> sexpr <> sexpr') sexprs)
    and runs = function
      | [] -> []
      | (sexpr :: sexprs) -> run sexpr sexprs
      in fun sexprs -> runs sexprs;;
      ...
      | const :: table' -> 
        (match const with
          | (sexpr', loc, _) -> 
              if sexpr = sexpr'
              then loc
              else run sexpr table')  
      | [] -> raise (X_this_should_not_happen "sub constants before constants.")
    in fun sexpr table ->
      run sexpr table;;
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmVarDef' (Var' (v, Free), expr') ->
         ...
         ^ (Printf.sprintf "\tmov qword [%s], rax\n" label)
         ^ "\tmov rax, sob_void\n"
      | ScmVarDef' (Var' (v, Param minor), expr') -> raise X_not_yet_supported
      | ScmVarDef' (Var' (v, Bound (major, minor)), expr') -> raise X_not_yet_supported
      | ScmBox' (Var' (v, Param minor)) -> 
        "\tmov rdi, 8\n"
        ^ "\tcall malloc\n"
        ^ (Printf.sprintf "\tmov r8, qword [rbp + 8 * (4 + %d)]\n" minor)
        ^ "\tmov qword [rax], r8\n"
      | ScmBox' _ -> raise (X_this_should_not_happen "box on none param")
      | ScmBoxGet' var' ->
      ...
      | ScmLambda' (params', Opt opt, body) -> 
        let label_loop_env = make_lambda_opt_loop_env ()
         ...
         ^ (Printf.sprintf "\tmov SOB_CLOSURE_CODE(rax), %s\n" label_code)
         ^ (Printf.sprintf "\tjmp %s\n" label_end)
         ^ (Printf.sprintf "%s:\t; lambda-optional body\n" label_code)
         ^ "\tmov r9, 2\n"
         ^ "\tadd r9, qword [rsp + 8 * 2]\n"
         ^ "\tmov rdx, sob_nil\n"
         ^ (Printf.sprintf "\tcmp qword [rsp + 8 * 2], %d\n" (List.length params'))
         ^ (Printf.sprintf "\tje %s\n" label_arity_exact)
         ^ (Printf.sprintf "\tjg %s\n" label_arity_more)
         ...
         ^ "\tcmp r9, 0\n"
         ^ (Printf.sprintf "\tjl %s\n" label_arity_ok)
         ^ "\tmov r8, qword [rsp + 8 * r9]\n"
         ^ "\tmov qword [rsp + 8 * r9], rdx\n"
         ^ "\tmov rdx, r8\n"
         ^ "\tdec r9\n"
         ^ (Printf.sprintf "\tjmp %s\n" label_arity_exact)
         ^ (Printf.sprintf "%s:\n" label_arity_more)
         ...
         ^ "\tmov rdx, rax\n"
         ^ "\tdec r9\n"
         ^ (Printf.sprintf "\tcmp r9, %d\n" ((List.length params') + 2))
         ^ (Printf.sprintf "\tjne %s\n" label_arity_more)
         ^ "\tmov r8, 2\n"
         ^ "\tadd r8, qword [rsp + 8 * 2]\n"
         ^ (Printf.sprintf "%s:\t; lambda-optional shrink loop\n" label_shrink_loop)
         ^ "\tcmp r9, -1\n"
         ^ (Printf.sprintf "\tje %s\n" label_shrink_end)
         ^ "\tmov qword [rsp + 8 * r8], rdx\n"
         ^ "\tmov rdx, qword [rsp + 8 * r9]\n"
         ^ "\tdec r9\n"
         ^ "\tdec r8\n"
         ^ (Printf.sprintf "\tjmp %s\n" label_shrink_loop)
         ^ (Printf.sprintf "%s:\t; lambda-optional shrink end\n" label_shrink_end)
         ^ "\tcmp r8, -1\n"
         ^ (Printf.sprintf "\tje %s\n" label_arity_ok)
         ^ "\tpop r9\n"
         ^ "\tdec r8\n"
