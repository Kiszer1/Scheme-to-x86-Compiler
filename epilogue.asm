...
L_code_ptr_bin_apply:
        ...
        mov r15, qword [rsp + 8 * 3] ; Closure
        cmp byte [r15], T_closure
        jne L_error_non_closure
        mov r8, qword [rsp + 8 * 4]             ; List
        mov r9, qword [rsp + 8 * 0]             ; Ret
        mov rcx, 0                              ; List Counter
.L_list_test:
        cmp byte [r8], T_nil
        je .L_list_ok
        cmp byte [r8], T_pair  
...
.L_list_ok:
        mov r8, qword [rsp + 8 * 4]             ; List
        lea rbx, [rcx * 8 - 8 * 2]            
        sub rsp, rbx                            ; Setting new stack size
        mov qword [rsp + 8 * 2], rcx            ; Pushing arg count
        mov qword [rsp + 8 * 0], r9             ; Pushing ret adress 
        mov r9, SOB_CLOSURE_ENV(r15)            ; Getting environment
        mov qword [rsp + 8 * 1], r9             ; Pushing environment
        mov r9, 0                               ; Counter
.L_list_loop:
        cmp rcx, 0
        je .L_apply_done
        mov rbx, SOB_PAIR_CAR(r8)               ; Getting arg
        mov r8, SOB_PAIR_CDR(r8)                ; Next arg
        mov qword [rsp + (8 * r9) + (8 * 3)], rbx  ; Pushing args
        dec rcx
        inc r9
        jmp .L_list_loop
.L_apply_done:
        jmp SOB_CLOSURE_CODE(r15)
